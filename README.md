...
+ OR **vim-bindings**: scroll up/down in history with `alt-l` and `alt-k`. Faster with `alt-u`/`alt-d`.
...
Note that some special characters may appear truncated if too wide. You might want to manually set your prefered emoji/special character font to a lower size in the `config.h` file to avoid this. By default, Twemoji is used at a smaller size than the usual text.
